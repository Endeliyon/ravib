<?php
	...
	class cms_user_model extends model {
		...
		public function get_users($order, $offset, $limit) {
			$query = "select u.* from users u order by %S,%S limit %d,%d";

			if (($users = $this->db->execute($query, $order, $offset, $limit)) === false) {
				return false;
			}
		...
		public function create_user($user) {
			$keys = array("id", "organisation_id", "username", "password", "one_time_key", "status", "fullname", "email", "last_login");
